...
install() {
  ...
  kubectl completion bash >"${SCRIPT_DIR}/../sources/kubectl-completion.sh"

  # renovate: datasource=github-releases depName=helm/helm
  local HELM_VERSION="v3.15.1"
  ...
  if command -v helm >/dev/null 2>&1; then
    helm completion bash >"${SCRIPT_DIR}/../sources/helm-completion.sh"
  else
  ...
  local KUBESEAL_VERSION="v0.26.3"
  ...
  kmux completion bash >"${SCRIPT_DIR}/../sources/kmux-completion.sh"
}
