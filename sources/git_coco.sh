...
git_coco() {
  ...
  _git_coco_configure_remote_api() {
    case $(printf "%s" "${GIT_REPOSITORY}" | jq -r '.url') in
    "github.com")
      github_http_init
      ;;
    "codeberg.org")
  ...
  _git_coco_get_remote_api() {
    case $(printf "%s" "${GIT_REPOSITORY}" | jq -r '.url') in
    "github.com")
      printf "https://api.github.com"
      ;;
    "codeberg.org")
      printf "https://codeberg.org"
      ;;
    esac
  }
  ...
  _git_coco_get_issue() {
    ...
    http_request "$(_git_coco_get_remote_api)/repos/$(printf "%s" "${GIT_REPOSITORY}" | jq -r '.owner + "/" + .name')/issues?per_page=100"
  ...
  get_co_author() {
    ...
    http_request "$(_git_coco_get_remote_api)/repos/$(printf "%s" "${GIT_REPOSITORY}" | jq -r '.owner + "/" + .name')/contributors?per_page=100"
    ...
    http_request "$(_git_coco_get_remote_api)/users/${CO_AUTHOR}/events?per_page=100"
    if [[ ${HTTP_STATUS} != "200" ]]; then
  ...
  local COMPONENT=""
  COMPONENT="$(cat <(_git_coco_guess_component) <(printf -- "None") | fzf --height=20 --ansi --reverse --prompt='Component:')"
  ...
  if var_confirm "Link issue"; then
    ISSUES_STRING="$(_git_coco_get_issue)"
  fi
