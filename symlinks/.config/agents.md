...
- **Tests**: Don't make external test package or black-box testing. Write the tests in the same package.
- **Tests**: Always run `go test -race` (include the `-race` flag)
- **Mocks**: When mocking is needed, use `//go:generate mock` directives and generated mocks. Never write mocks or stubs manually
...
- **Readability**: Add blank lines to separate logical groups of statements (e.g. after variable declarations, before return statements, between distinct steps of a function, around loops and conditionals). Never write dense walls of code. Vertical whitespace is mandatory for human readability.
...
- Always use the Atlassian MCP server to fetch Jira tickets, never guess or rely on cached content
